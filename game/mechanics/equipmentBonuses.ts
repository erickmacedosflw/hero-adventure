...
export const getEquipmentBonuses = (item: Item | null): EquipmentBonuses => {
  if (!item) {
    return { atk: 0, def: 0, speed: 0, maxHp: 0, maxMp: 0 };
  }
  ...
  if (item.type === 'weapon') {
    return { atk: base, def: 0, speed: 0, maxHp: 0, maxMp: 0 };
  }
  if (item.type === 'armor') {
    return { atk: 0, def: base, speed: 0, maxHp: 0, maxMp: mpBonus };
  }
  if (item.type === 'helmet') {
    return { atk: 0, def: base, speed: 0, maxHp: hpBonus, maxMp: 0 };
  }
  if (item.type === 'legs') {
    return { atk: 0, def: 0, speed: base, maxHp: 0, maxMp: 0 };
  }
  if (item.type === 'shield') {
    return { atk: 0, def: base, speed: 0, maxHp: 0, maxMp: 0 };
  }
