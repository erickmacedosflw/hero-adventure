var editor = (function () {
  ...
  class Dropdown {
      ...
      */
      render(view) {
          ...
          let wrap = crelt("div", { class: prefix$1 + "-dropdown-wrap" }, btn);
          let open = null;
  ...
  class DropdownSubmenu {
      ...
      */
      render(view) {
          ...
          let wrap = crelt("div", { class: prefix$1 + "-submenu-wrap" }, btn, crelt("div", { class: prefix$1 + "-submenu" }, items.dom));
          let listeningOnClose = null;
          let openSubmenu = (e) => {
              e.preventDefault();
              e.stopPropagation();
  ...
  */
  ...
  function markItem(markType, options) {
      let passedOptions = {
          active(state) { return markActive$1(state, markType); }
      };
      for (let prop in options)
          passedOptions[prop] = options[prop];
  ...
  function linkItem(markType) {
      return new MenuItem({
          ...
          run(state, dispatch, view) {
              if (markActive$1(state, markType)) {
  ...
  */
  ...
  const iContent = function(el){
      ...
      return (before && before !== 'none')? before.replace(/^['"]/, '').replace(/['"]$/, '') : ""
          + el.textContent
          + (after && after !== 'none')? after.replace(/^['"]/, '').replace(/['"]$/, '') : "";
  };
  ...
  const PFSchema = new Schema({
      nodes: {
          ...
          glyph: {
              ...
              toDOM: node => ["i", {class: "pf pf-"+node.attrs.kind+" pf-"+node.attrs.glyph}],
              parseDOM: [{tag: "i", getAttrs: node => {
                  return {
                      kind: node.classList.contains("pf-wide")? "wide" :
                          node.classList.contains("pf-backfill")? "backfill" :
                              "glyph",
                      glyph: node.getAttribute("class").substr(node.getAttribute("class").lastIndexOf(" pf-")+4)
                  };
              }}]
          },
          doc: {
      ...
      marks: {
          color: {
              ...
              parseDOM: [{style: "color", getAttrs(val) { return {color: val} }}]
          },
      }
  });

  const PFKeymap = {
      "Ctrl-b": toggleMark(PFSchema.marks.background),
      "Ctrl-l": toggleMark(PFSchema.marks.wide),
      "Ctrl-Space": function(state, dispatch) {
          let {doc, selection} = state;
          if (selection.empty) return false
          let attrs = null;
          if (!doc.rangeHasMark(selection.from, selection.to, PFSchema.marks.color)) {
              attrs = {color: prompt("What colour?", "")};
              if (!attrs.color) return false
          }
          return toggleMark(PFSchema.marks.color, attrs)(state, dispatch)
      }
  };

  const handleColor = (callback) => {
      const icon = document.querySelector('.ProseMirror-menubar .color');
      let panel = icon.nextElementSibling;

      const handleClose = (icon, panel) => {
          icon.classList.remove('panel-open');
          if (panel) panel.remove();
      };

      if (panel && panel.classList.contains('panel')) {
          handleClose(icon, panel);
      } else {
          icon.classList.add('panel-open');
          panel = constructElement("form", {
              classes: ["panel"],
              elements: {
                  'input': {attributes: {
                      type: 'color',
          ...
          panel.addEventListener('submit', (ev) => {
              ev.preventDefault();
              callback({color: panel.querySelector("input").value});
              handleClose(icon, panel);
          });
          icon.parentNode.insertBefore(panel, icon.nextSibling);
      }
  };
  ...
  const isGlyph = (state, kind) => {
      const { from, to, empty } = state.selection;
      if (empty){
          if (0<from) from--;
          else if (to<state.doc.content.size) to++;
          else return false;
      }
      let found = false;
      state.doc.nodesBetween(from, to, (node)=>{
          if(node.type.name == 'glyph' && node.attrs.kind == kind)
              found = true;
          else if(found)
              found = 'some';
      });
      return (found===true)? 'all' : found;
  };

  const setGlyphKind = (state, dispatch, kind) => {
      const { from, to, empty } = state.selection;
      if (empty){
          if (0<from) from--;
          else if (to<state.doc.content.size) to++;
  ...
  const makeIcon = (icon) =>
      () => constructElement("button", {
          classes: ["ProseMirror-icon"],
          html: '<i class="pf pf-'+icon+'"></i>',
      });
  ...
  const PFMenu = [
      [
          new MenuItem({
              title: "Undo last change",
              render: makeIcon('icon-undo'),
              enable: state => undo(state),
              run: undo,
          }),
          new MenuItem({
              title: "Redo last change",
              render: makeIcon('icon-redo'),
              enable: state => redo(state),
              run: redo,
          })
      ],
      [
          new MenuItem({
              title: 'Increase Size',
              render: makeIcon('icon-size-increase'),
              run(state) {
                  const view = document.querySelector(".ProseMirror");
                  view.style.fontSize = parseFloat(view.style.fontSize)*1.5 + "em";
              }
          }),
          new MenuItem({
              title: 'Decrease Size',
              render: makeIcon('icon-size-decrease'),
              run(state) {
                  const view = document.querySelector(".ProseMirror");
                  view.style.fontSize = parseFloat(view.style.fontSize)/1.5 + "em";
              }
      ...
      [
          new MenuItem({
              title: 'Color',
              class: 'color',
              render: makeIcon('icon-sun'),
              active(state) { return markActive(state, PFSchema.marks.color); },
              enable(state) { return !state.selection.empty; },
              run(state, dispatch, view) {
                  handleColor((attrs) => {
                      toggleMark(PFSchema.marks.color, attrs)(view.state, view.dispatch);
                      view.focus();
                  });
              },
          }),
          new MenuItem({
              title: 'Wide Alternate',
              class: 'wide',
              render: makeIcon('icon-resize-horizontal'),
              active: (state)=> isGlyph(state, 'wide'),
              enable: (state)=> !state.selection.empty,
              run(state, dispatch) { setGlyphKind(state, dispatch, isGlyph(state, 'wide')=='all'? 'glyph' : 'wide'); },
          }),
          new MenuItem({
              title: 'Backfill Variant',
              class: 'backfill',
              render: makeIcon('icon-background'),
              active: (state)=>isGlyph(state, 'backfill'),
              enable: (state)=>!state.selection.empty,
              run(state, dispatch) { setGlyphKind(state, dispatch, isGlyph(state, 'backfill')=='all'? 'glyph' : 'backfill'); },
          })
      ],
  ];
  ...
  function setup(el){
      ...
      placeholder.innerText = "Compose...";
      let editor = document.createElement("div");
      [...el.attributes].forEach(attr=>editor.setAttribute(attr.nodeName,attr.nodeValue));
      el.replaceWith(editor);
      el.compose = (glyph)=>{
          let state = editor.view.state;
          glyph = PFSchema.nodes.glyph.create({
              'kind': 'glyph',
              'glyph': glyph.dataset.codeName
          });
          if(state.doc.textContent == "Compose..."){
              editor.view.dispatch(state.tr.replaceWith(0, state.doc.content.size, glyph));
          } else {
              editor.view.dispatch(state.tr.insert(state.selection.from, glyph));
          }
      };
      Object.defineProperty(el, "value", {
          get: ()=>{
              let text="";
              for(let i of editor.querySelectorAll(".ProseMirror i"))
                  text += iContent(i);
              return text;
          }
      });
      editor.view = new EditorView(editor, {
          state: EditorState.create({
              doc: DOMParser.fromSchema(PFSchema).parse(placeholder),
              plugins: [
                  ...exampleSetup({
                      schema: PFSchema,
                      menuContent: PFMenu,
                  ...
                  keymap(PFKeymap),
                  new Plugin({view(){return {update(view){
                      el.dispatchEvent(new Event("change"));
                  }}}})
              ]})});
      editor.querySelector(".ProseMirror").style.fontSize = "1.0em";
      return editor;
  }
